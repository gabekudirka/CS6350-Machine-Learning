...
class DecisionTree:
    ...
    def build_tree(self, purity_type='entropy', max_depth=16):
        ...
        self.total_purity = self.purity_function(self.labels)
    ...
    def id3(self, examples, attributes, node, depth):
        ...
        split_attribute = self.select_split_attribute(attributes, examples)
    ...
    def calculate_info_gain(self, examples, attribute):
        ...
        total = len(self.examples.index)
        gain = 0
        for val in unique_vals:
            examples_with_val = examples[examples[attribute] == val]
            purity = self.purity_function(examples_with_val['label'])
            weighted_purity = (value_counts[val]/total)*purity
